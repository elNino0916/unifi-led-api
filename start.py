...
Usage:
  ...
  python start.py discover        # Auto-discover all UniFi devices and their IDs
...
def resolve_device_ids(base_dir: Path, config: AppConfig, group: str | None = None) -> list[str]:
    ...
    with groups_file.open("r", encoding="utf-8") as f:
        try:
            ...
            raise ValueError(f"Failed to parse {groups_file.name}. Ensure it is valid JSON.")
...
async def process_device(
    session, config, base_dir, device_id, command, target_state=None, dry_run=False, color=None, brightness=None
):
    ...
    if command == "led":
        ...
        if "led_override" not in dev_cfg:
            logging.warning("Device '%s' lacks native LED override explicitly. Skipping to prevent Gateway/Switch corruption.", dev_cfg.get("name", device_id))
            return
...
async def handle_led_webhook(request):
    ...
    try:
        tasks = [
            process_device(session, config, base_dir, device_id, "led", state, False, color, brightness)
...
async def async_main(args):
    ...
    try:
        ...
        if args.command == "discover":
            logging.info("Discovering devices on controller %s (site: %s)", config.controller, config.site)
            devices = await led_logic.discover_devices(session, config.controller, config.site, timeout=config.timeout)
            print("\n" + "=" * 90)
            print(f"{'DEVICE NAME':<25} | {'MAC ADDRESS':<17} | {'MODEL':<15} | {'TOGGLE':<6} | {'DEVICE ID'}")
            print("-" * 90)
            for d in devices:
        ...
        tasks = [
            process_device(
                session, config, base_dir, device_id, args.command, target_state, dry_run, color, brightness
            )
            for device_id in device_ids
        ]
        await asyncio.gather(*tasks)
...
def main():
    ...
    parser.add_argument("--device", help="Target a specific device ID or comma-separated list of IDs")
    ...
    led_parser.add_argument("--brightness", type=int, choices=range(0, 101), metavar="[0-100]", help="Brightness percentage 0-100 for supported APs")
